...
*/
        ...
        if ($cek->num_rows() > 0) {
            $data_login = $cek->row();
            $data_akun = $this->db->get_where('daftar_akun', ['id' => $data_login->npm])->row();
            if ($data_akun->keterangan == "Belum Diverifikasi" || $data_akun->keterangan == "Akun Dibatalkan") {
                $return = ['status' => False, 'pesan' => 'Akun anda belum diverifikasi oleh operator kemahasiswaan'];
            } else {
                $return = ['status' => True, 'data_login' => $data_login, 'nama_lengkap' => $data_akun->nama_lengkap];
            }
        } else {
            $return = ['status' => False, 'data_akun' => '', 'pesan' => 'Username dan password tidak ditemukan di sistem!'];
        }
        return $return;
    }
}
